//! Local declarations of the VEXos jumptable symbols used by this SDK.
//!
//! The upstream `velox_jumptable` package declares `[*c]` pointers to
//! `opaque` types (e.g. `[*c]FIL`) and returns `opaque` values by value
//! (e.g. `FRESULT`), neither of which Zig 0.16.0 permits. These redeclarations
//! use the ABI-equivalent forms: opaque handles become `?*anyopaque` and the
//! FatFs `FRESULT` enum becomes `c_int`.
//!
//! Only the symbols referenced by `Io.zig` are declared here. The declarations
//! are `extern const` (a global holding a function pointer), matching the
//! jumptable convention; the linker resolves the symbol to the function
//! exported by VEXos.
//!
//! Host builds link a stub library that exports the same symbols, so this
//! module also compiles for native targets.
